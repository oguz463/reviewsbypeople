<x-app-layout>
<x-slot name="head">
  <title>{{config('app.name') . ' - ' . __('Search Results')}}</title>
  <meta name="description" content="{{config('app.name') . ' - ' . __('Search Results')}}">
</x-slot>
<div class="max-w-4xl px-6 mx-auto my-8">
  <h1 class="text-3xl font-bold mb-8 text-center">"{{$query}}" <span class="uppercase">{{__('Search Results')}}</span></h1>
